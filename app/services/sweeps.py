...
def _enqueue_committed(rows: Sequence[Any], enqueue: Enqueue, sweep: str) -> None:
    """Enqueue each committed row; a failure is logged and left for a later reconcile sweep."""
    for row in rows:
        try:
            enqueue(str(row.file_id), str(row.organization_id))
        except Exception as exc:
            # The row is committed as `uploaded` with an old (stale sweep) or fresh (reconcile) uploaded_at,
            # so a later reconcile sweep re-enqueues it.
...
def stale_sweep(db: Session, *, stale_after_seconds: int, max_attempts: int,
    ...
    """Recover files whose worker stopped heartbeating: reset and re-enqueue, or error when attempts are spent."""
    ...
    _enqueue_committed(reset, enqueue, "stale")
    log.info("stale_sweep", reset=len(reset), errored=len(errored),
...
def reconcile_sweep(db: Session, *, reconcile_after_seconds: int, max_attempts: int,
    ...
    db.commit()                          # enqueue only after the state change is durable
    _enqueue_committed(rows, enqueue, "reconcile")
    log.info("reconcile_sweep", requeued=len(rows), errored=len(errored),
