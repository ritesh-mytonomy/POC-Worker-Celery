...
| 1.4 | Tidy-up for fewer files, behaviour unchanged. Final layout: `engine/file_checks.py` (limits, Rejected, detection, archive checks, extraction); `workers/tasks.py` (Celery app, Heartbeat, process_upload/document/archive, sweeper tasks) and `workers/clients.py` (Internal API client, S3 store, and `Transient`, moved from the engine — the only place that raises it); `poc/main.py` (api entry point: `app.main:app` + `/poc/*`, `enqueue_scan_stub`) and `poc/worker.py` (worker-scan entry point: `workers.tasks` + the scan route and `scan_stub`), with `TASK_SCAN_STUB` in `poc/__init__.py`. The dependency points one way — `poc/` imports `app/`/`workers/`, never the reverse; the scan worker never loads the FastAPI app and the API never loads the worker app (all tested). Commands: `api` runs `poc.main:app`, `worker-ingest`/`worker-maint` run `workers.tasks`, `worker-scan` runs `poc.worker`. Task names are unchanged and set explicitly (tested: exactly `workers.ingest.process_upload`, `workers.sweepers.run_stale_sweep`, `workers.sweepers.run_reconcile_sweep`, plus `workers.scan.scan_stub` on the scan worker); each section keeps its original logger name. The Celery `safe_say` patch is removed: R15.1 now covers the lines our code emits, Celery's shutdown notices may be plain text, and log readers skip non-JSON lines. Tasks 7.2 and 12.1 keep their original paths as history | R15.1 · design §2, §3, §6.2a, §7, §8 · task 7.2, 12.1 |
