...
def main() -> int:
    ...
    try:
        ...
        while True:
            files = {f["file_id"]: f["status"] for f in lib.batch(batch_id)}
            now = time.time() - started
            polls += 1
            busy = [f for f, st in files.items() if st == "processing"]
            max_processing = max(max_processing, len(busy))
            for f in busy:
                first_processing.setdefault(f, now)
            if first_done is None and any(st == "processed" for st in files.values()):
                first_done = now
            if all(st in lib.TERMINAL for st in files.values()) or now > 3 * 30 + 90:
                break
            time.sleep(0.5)
        ...
        since = [l for l in lib.worker_lines("api") if lib.ts(l) >= started]
        ...
        lost = sum(1 for l in lib.worker_lines("worker-ingest") if l.get("event") == "claim_lost"
                   and l.get("file_id") in ids and lib.ts(l) >= started)
